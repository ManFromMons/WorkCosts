...
5. Inbox on `main` via `update-to-review` / `Update-ToReviewOnMain.ps1`. When tests pass, set that heading **Status** to `ready-for-review` with a **Work summary**, **Questions**, and **Deviations**, then open the squash PR. Do not squash-merge until the heading is **Status** `done`.
