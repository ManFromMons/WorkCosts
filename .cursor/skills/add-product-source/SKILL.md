...
Then follow `implement-feature` / `update-to-review` (inbox on `main`; **open the squash PR at `ready-for-review`**; do not squash-merge until that heading is `done`). Branch `feature/source-<host>-<Title>` from `origin/main`.
...
7. Re-run tests. When they pass, land to-review **Status** `ready-for-review` via `Update-ToReviewOnMain.ps1` with a **Work summary** and every **Deviation**. Open the squash PR. Do not squash-merge until **Status** `done` on that inbox heading.
