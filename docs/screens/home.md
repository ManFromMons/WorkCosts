...
`WorkJobs` include Job + Items, newest first. **Instances only** (`IsDefinition == false`); definition rows belong to a Job recipe ([15-workjob-job-subset.md](../features/15-workjob-job-subset.md)) and must not appear as cards. SQLite cannot `ORDER BY` DateTimeOffset directly; use UTC.
