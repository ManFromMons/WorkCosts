...
- **Seq:** 17
- **Depends-on:** `garage-job-interval-logic`, `cars`, `workjob-job-subset`
- **Status:** draft
- **PR:** none
...
- Seq **17**; Depends-on interval logic, cars, **workjob-job-subset**. `WorkJob` ≠ `ItemOfWork`. `WorkJob` belongs to `Job`. Status **draft**. Refine later.
