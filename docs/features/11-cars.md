...
- Persist **`VehicleOrderJson`** now (empty until [13-car-vin-lookup.md](13-car-vin-lookup.md)). Extra lookup facts stay in that JSON; they do not replace nickname / make / model / model-number / year / engine on the row.
...
- **Out of scope:** Calling mdecoder or FastCarCheck. **Car types** catalogue UI ([12-car-details.md](12-car-details.md)) and encoding a type list in the repo ([16-car-details-seed.md](16-car-details-seed.md)). Work-job definition copy ([15-workjob-job-subset.md](15-workjob-job-subset.md)). GarageJob collation / ItemOfWork UI ([17-item-of-work-ui.md](17-item-of-work-ui.md), later). New Home. Zip import implementation (document merge keys only). GNOME/iPad UI.
...
| Table | Column | Rules |
| :--- | :--- | :--- |
| `GarageJobs` | `CarId` `Guid?` | FK → `Cars`, **Restrict**. Null allowed for rows created before this Seq. New garage-job writes in later UI require a car ([17-item-of-work-ui.md](17-item-of-work-ui.md)). `TargetKind` + `TargetLabel` **stay**. |
| `WorkJobs` | `CarId` `Guid?` | FK → `Cars`, **Restrict**. Null allowed for existing work jobs. Later UI requires a car on new work. |
| `ItemsOfWork` | `CarId` `Guid?` | FK → `Cars`, **Restrict**. Null allowed for existing completions. Later writes also set car-details ([12-car-details.md](12-car-details.md)). |
...
- Feature id `11-cars`; Seq **11**; Depends-on `none`.
