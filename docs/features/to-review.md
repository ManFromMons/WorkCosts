...
- **Seq:** 11
- **Status:** done
- **Change set:** branch `feature/11-cars-Cars` Ôö£├ÂÔö£├ºÔö£├é [https://github.com/ManFromMons/WorkCosts/pull/13](https://github.com/ManFromMons/WorkCosts/pull/13)
- **Last note:** Feature file Status is `done`. PR #13 is ready.

### Work summary

- Stuff Ôö£├ÂÔö£├æÔö£├Ñ Cars master/detail, trailing Add. Narrow width stacks the list, then the detail, with Back to the list. Detail fields are three per row.
...
- [x] The image chooser reuses `ProductImagePicker.ChooseFromCandidatesAsync` (a ContentDialog). An optional title lets cars say Ôö£├ÂÔö£├ºÔö¼├║Select a photoÔö£├ÂÔö£├ºÔö£├┐. Add Car stays a sheet. Chromium runs before that dialog.
...
- **Seq:** 12
- **Status:** done
- **Change set:** branch `feature/12-car-details-Car-types` Ôö£├ÂÔö£├ºÔö£├é [https://github.com/ManFromMons/WorkCosts/pull/14](https://github.com/ManFromMons/WorkCosts/pull/14)
- **Last note:** Squash-merged to `main` as `a230139` (#14). Feature file Status is `done`.

### Work summary

- Stuff Ôö£├ÂÔö£├æÔö£├Ñ Car types master/detail, trailing Add. Narrow width stacks the list, then the detail, with Back to the list.
...
- **Seq:** 13
- **Status:** done
- **Change set:** branch `feature/13-car-vin-lookup-VIN-lookup` Ôö£├ÂÔö£├ºÔö£├é [PR #16](https://github.com/ManFromMons/WorkCosts/pull/16)
...
- First request is HttpClient GET `https://www.mdecoder.com/decode/{vin}`. A Cloudflare / robot-check body opens the same off-dialog Chromium path as Autodoc. Polling stays on the sheet: "Requesting mdecoderÔö£├ÂÔö£├ºÔö¼┬¼", then "Waiting, retrying in 30sÔö£├ÂÔö£├ºÔö¼┬¼". Cancel stops polling and does not discard the form.
...
- **Status:** draft
- **Change set:** none
- **Last note:** Landed on main via merge-planning. **Draft Ôö£├ÂÔö£├ºÔö£├é resume later.** Filename is `14-car-fastcarcheck.md`.
...
- **Seq:** 15
- **Status:** done
- **Change set:** branch `feature/15-workjob-job-subset-Work-job-subset` Ôö£├ÂÔö£├ºÔö£├é [PR #17](https://github.com/ManFromMons/WorkCosts/pull/17)
...
- **Seq:** 16
- **Status:** done
- **Change set:** branch `feature/16-car-details-seed-Car-details-seed` Ôö£├ÂÔö£├ºÔö£├é [PR #19](https://github.com/ManFromMons/WorkCosts/pull/19)
...
- **Status:** draft
- **Change set:** none
- **Last note:** Landed on main via merge-planning. **Draft Ôö£├ÂÔö£├ºÔö£├é refine later.** Filename is `17-item-of-work-ui.md`.
...
- **Status:** done
- **Change set:** branch `cursor/garage-job-interval-logic-f3f1` Ôö£├ÂÔö£├ºÔö£├é [https://github.com/ManFromMons/WorkCosts/pull/11](https://github.com/ManFromMons/WorkCosts/pull/11)
- **Last note:** Squash-merged to `main` as `016a199` (#11). Feature file Status is `done`.

### Work summary

- Persist `ItemOfWork` completions (`OccurredAt`, optional odometer miles Ôö£├ÂÔö£┬¢Ôö£├ª 0) with Restrict FK. `ItemOfWorkCommands` create / list / latest / delete. `GarageJobCommands.TryDeleteAsync` returns `HasCompletions` and keeps the parent.
- Persist `GarageJob.IntervalAnchorDate` (`DateOnly?`) via `UpdateAsync`. Empty anchor + no completions Ôö£├ÂÔö£├æÔö£├Ñ `DueImmediately`.
...
- `GarageJobRollupCalculator`: each `ProductJob` quantity 1, merge required products, garage ├ö├Â┬╝Ôö£Ôòæ + DIY ├ö├Â┬╝Ôö£Ôòæ to 2 dp `AwayFromZero`, referenced job duration (parent duration echoed, not added). `IsAllJobs` excluded unless linked.
...
- [x] Branch `cursor/garage-job-interval-logic-f3f1` (cloud-agent prefix) instead of `feature/garage-job-interval-logic-Ôö£├ÂÔö£├ºÔö¼┬¼`.
...
- [x] Sample 1 live `.product__new-price` on 2026-08-21 was **Ôö¼Ôöñ├ö├Â├ëÔö¼├│49.96**; fixtures lock the confirmed **Ôö¼Ôöñ├ö├Â├ëÔö¼├│50.24**.
...
- [x] Sample 2 fixture uses the confirmed unit price **├ö├Â┬╝Ôö£Ôòæ98.50**; live HttpClient HTML on 2026-08-21 showed **├ö├Â┬╝Ôö£Ôòæ103.30** (RRP ├ö├Â┬╝Ôö£Ôòæ109.09). Tests no longer lock a GBP amount.
...
- [x] Manufacturer is the first token of `brandImage` alt so Ôö£├ÂÔö£├ºÔö¼├║Eicher PremiumÔö£├ÂÔö£├ºÔö£├┐ matches confirmed **Eicher**.
...
- [x] Added `InputToolTip.Bind(ComboBox, Ôö£├ÂÔö£├ºÔö¼┬¼)` so Technology matches the other extra-spec tooltips.
