...
XML dump of the relational catalogue (categories, jobs, products, product-jobs, equivalents, work jobs, work-job items, cars, garage jobs, items of work). Use stable element names matching CLR properties. Include product and car `Id` values so images can match. Currency is GBP; do not localize numbers in XML (invariant `decimal` with `.`).
...
| Car | `Id` | Keep destination row, including `DeletedAt` | Insert car row and copy `images/cars/{id}.{ext}` |
